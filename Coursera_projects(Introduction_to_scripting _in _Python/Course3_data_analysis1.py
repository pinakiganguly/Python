...
##
## Part 1: Functions to compute op batting statistics by year
##
...
def filter_by_year(statistics, year, yearid):
    ...
    for line in statistics:
        if int(line[yearid]) == year:
            yearwise_filter.append(line)
    return yearwise_filter
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    top_players = []
    for line in statistics:
        tup = (line[info["playerid"]], formula(info, line))
        top_players.append(tup)
    top_players.sort(key=lambda tup: tup[1], reverse=True)
    return top_players[0:numplayers]
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    fieldname = info["playerid"]
    firstname = info["firstname"]
    lastname = info["lastname"]
    baseballinfo = read_csv_as_nested_dict(filename, fieldname, info["separator"], info["quote"])
    for tup in top_ids_and_stats:
        name = baseballinfo[tup[0]][firstname] + " " + baseballinfo[tup[0]][lastname]
        stat = "{:.3f}".format(tup[1])
        combined = stat + " --- " + name
        player_names.append(combined)
    return player_names
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    """
    top_stats = read_csv_as_list_dict(info["battingfile"], info["separator"], info["quote"])
    year_filter = filter_by_year(top_stats, year, info["yearid"])
    top_players = top_player_ids(info, year_filter, formula, numplayers)
    players_stats = lookup_player_names(info, top_players)
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    dic = {}
    for line in statistics:
        if not line[playerid] in dic:
            dic[line[playerid]] = {playerid: line[playerid]}
        for agg in fields:
            if not agg in dic[line[playerid]]:
                dic[line[playerid]][agg] = 0
            dic[line[playerid]][agg] += int(line[agg])
    return dic
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    """
    aggregate = []
    top_career_stat = read_csv_as_list_dict(info["battingfile"], info["separator"], info["quote"])
    agg = aggregate_by_player_id(top_career_stat, info["playerid"], info["battingfields"])
    for value in agg:
        aggregate.append(agg[value])
    top_id = top_player_ids(info, aggregate, formula, numplayers)
    top_stats = lookup_player_names(info, top_id)
    return top_stats
